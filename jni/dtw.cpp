...
//     typedef int value_t;
    ...
    typedef vector_unit<value_t,VW> vu;
...
//        os << a[i] << " " << b[i] << "\n";
    ...
    if( true )
    {
        ...
        for( int i = 0; i < 1000; i++ ) {
            ...
            const value_t large_value = vu::LARGE_VALUE;
            res = dtw_align( a.begin(), a.end(), b.begin(), b.end(), large_value, fabsf, my_min3<value_t> );
            ncup += a.size() * b.size();
        }
        std::stringstream ss;
        ss << ncup << " in " << t1.elapsed() << ": " << (ncup / (t1.elapsed() * 1e9)) << "\n";
//        char *str = 0;
//		asprintf( &str, "%d %d %d in %f: %f\n", VW, res, ncup, t1.elapsed(), (ncup / (t1.elapsed() * 1e9)));


//		strlcat( buffer, str, 512 );
		strlcat( buffer, ss.str().c_str(), 512 );
//		free(str);
